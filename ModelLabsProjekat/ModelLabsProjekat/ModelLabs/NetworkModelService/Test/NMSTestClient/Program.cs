...
namespace FTN.Services.NetworkModelService.TestClient
{
	public class Program
	{
			...
			try
			{
				...
				do
				{
                    ...
                    else if (str == "5")
                    {
                        try
                        {
                            long globalId = 0x000000010000003d;
                            tgda.GetValues(globalId);
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("Greska kod dobijanja RC {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
                    ...
                    else if (str == "6")
                    {
                        try
                        {
                            tgda.GetExtentValues(ModelCode.RC);
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("Greska kod dobijanja RC {0}", ex.Message);
                    ...
                    else if (str == "7")
                    {
                        try
                        {
                            ...
                            List<long> lista = tgda.GetExtentValues(ModelCode.TAPCHANGER);
                            long najveci = lista.Max();
                            asocijacija.Type = 0;
                            asocijacija.PropertyId = ModelCode.TAPCHANGER_TAPSCHEDULE;
                            tgda.GetRelatedValues(najveci, asocijacija);
                            Console.WriteLine("\t*******Uspijesno pronadjeni svi TAPSCHEDULE od tapchanger\nTAPCHANGER sa najvecim ID-em je:" + najveci);
                            Console.WriteLine("\n");
                        ...
                        {
                            message = string.Format("Get TAPCHANGER failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
		...
		private static void PrintMenu()
		{
            ...
            Console.WriteLine("\t5) Prvi test GDA values metode ");
            Console.WriteLine("\t6) Prvi test GDA extend metode ");
            Console.WriteLine("\t7) Get TAPSCHEDULE associeted to TAPCHANGER with highest GID");
